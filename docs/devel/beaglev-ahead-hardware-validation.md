...
| CPU-001 | OPEN-HW | Exact C910 mvendorid, marchid, mimpid and custom CPU ID values? | Linux DT identifies thead,c910; QEMU has C906/C908 IDs but no C910 model. Reusing a C908 ID is forbidden. | M-mode CSR probe on each hart; repeat after warm reset. |
...
| CPU-003 | OPEN-HW | Custom CSR reset values, writable masks, privilege checks and inter-hart scope? | Current QEMU T-Head CSR layer is C908-oriented and returns zero for most declared CSRs. | Generated M/S/U CSR probe with read/write/read, illegal-access trap capture and warm/cold reset, restricted to documented safe fields. |
| CPU-004 | OPEN-HW | MAEE page-table encoding and memory-type semantics? | QEMU explicitly says MAEE is not implemented; boot firmware may rely on it for device/noncacheable mappings. | Bare-metal mappings over RAM and a harmless MMIO scratch target; test access, ordering, aliases and faults with/without MAEE. |
| CPU-005 | OPEN-HW | XTheadVector availability/version and feature identification on C910? | Board sources say Vector 0.7.1 and Linux uses xtheadvector with VLENB 16. The current public XTheadVector specification's availability text does not clearly name C910. | Read vector CSRs and run discriminating 0.7.1-versus-1.0 encodings on all harts, capturing illegal traps and results. |
| CPU-006 | OPEN-HW | Exact vector tail, mask, vstart, saturation, FP flags and fault-only-first edge behavior? | The extension spec gives architectural rules, but silicon errata/revision behavior is unknown. | Randomized differential suite with guarded pages and signal/trap recovery; keep per-stepping expected results. |
| CPU-007 | OPEN-HW | PMP entry count, grain, reset, locking and interaction with TEE/security extensions? | System documentation says 32 entries; software-visible details still need proof. | Safe M-mode PMP probe in disposable address ranges, including TOR/NA4/NAPOT and lock only where reset recovery is certain. |
...
| CPU-011 | OPEN-HW | Debug/security extensions and reset visibility needed by guest software? | System documentation mentions debug and interrupt/L2 security extensions without a complete public software contract. | Inventory OpenOCD/vendor debug behavior; probe only guest-visible registers and traps. |
