...
| MIG-001 | OPEN-DOC | Which hardware state is volatile, retained, derived or external at migration boundaries? | Physical hardware has no VM migration contract; QEMU still needs internally consistent snapshots. C910 custom CPU/vector/PMU state, C900 CLINT software bits/compares/architectural time, and C900 PLIC priorities, thresholds, pending/active/enables, input samples, trigger configuration and delegation state now have VMState descriptions. The CLINT qtest checks counter continuity and destination timer rearming. The PLIC qtest migrates an active interrupt with separately pending state and a held input, then proves completion and re-delivery on the destination. Other current and future devices still need equivalent boundary coverage. | Define per-device VMState from architectural state and test save/load before/after asserted interrupts at every phase; add CPU PMU/vector boundary cases and document derived state plus backend exclusions. |
