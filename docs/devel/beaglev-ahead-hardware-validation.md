...
| CPU-008 | OPEN-HW | PMU event numbers, counter width, overflow IRQ and inhibit/delegation behavior? | Mainline DT maps standard and raw events. The December 2024 physical-board boot capture reports 16 MHPM counters, matching QEMU's mask 0x0007fff8, but this does not prove event semantics or privilege filters. | Deterministic instruction/cache/branch workloads and overflow tests, normalized rather than cycle-count compared. |
