...
| MIG-001 | OPEN-DOC | Which hardware state is volatile, retained, derived or external at migration boundaries? | Physical hardware has no VM migration contract; QEMU still needs internally consistent snapshots. C910 custom CPU/vector/PMU state and the C900 CLINT software bits, compares and architectural time now have VMState descriptions; host timers are correctly derived from the architectural state after load. The CLINT qtest migrates pending software state plus asserted and future timer state, checks counter continuity, and proves destination timer rearming. Other current and future devices still need equivalent boundary coverage. | Define per-device VMState from architectural state and test save/load before/after asserted interrupts at every phase; add CPU PMU/vector boundary cases and document derived state plus backend exclusions. |
