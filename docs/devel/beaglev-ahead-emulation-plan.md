...
and GMAC DHCP/ping/HTTP-integrity traffic, PLL-polling, C910 FXCR, synthetic
C910 CSR VMState v1/v2 compatibility, genuine historical v1/v2
...
XTheadVector vector-permutation/slide checkpoint: 78ad4d6e56

XTheadVector vector-index-width/group checkpoint: 6d147eea4b

XTheadVector m8 gather checkpoint: 4f9c7e9893
...
RVV 0.7.1 slide-down rules also prohibit a masked destination from overlapping
``v0`` even at LMUL=1, overriding the generic LMUL=1 overlap allowance.  Raw
encodings require illegal-instruction traps for ``th.vslidedown.vx``,
``th.vslidedown.vi`` and ``th.vslide1down.vx`` with masked ``vd=v0``; the first
trap preserves nonzero ``vstart`` and all 16 ``v0`` bytes, while unmasked
``vd=v0`` and masked non-``v0`` controls remain legal.  The augmented guest
passes in normal, dependency-minimal and ASan/UBSan builds.  Isolated gather
and slide-checker mutations fail at exits 14 and 15.

Checkpoint ``1369cec4d9`` implements the DWMAC receive-interrupt
watchdog register used by Linux RX interrupt mitigation.  TH1520 uses the
pinned 500 MHz ``stmmaceth`` reset/reference rate, so Linux's default
``RIWT=0xa0`` expires after exactly 81,920 ns.  A DIC-suppressed terminal RX
descriptor arms the one-shot timer; expiry raises RI/NIS and the GMAC PLIC
...
scalar-RV64 oracle and mutations at exits 3, 9 and 13 remain valid historical
evidence.

An earlier checkpoint closed the pending eMMC tuning-interrupt
...
``../validation-artifacts/c910-historical-migration-20260825/capture_and_audit.py``
pins the last v1 producer commit
``cce292534e6cc52135c4c9180ca0c5bec5d31259`` and binary SHA-256
``f62a61633074a15a79603a97d2203070269aa74ce6c81257ae04535a7d71f25a``,
and the last v2 producer commit
``10321e0df1670ec24d420285b15ebf0bf57c0066`` and binary SHA-256
``60350184ec4cefd64144b40fe6895f652ceced9bdb191b2b0f75449753f09a2f``.
It emitted genuine four-hart streams and checked every seeded C910 field.  The
v1 stream is 9,489,378 bytes, SHA-256
``dc65b5540169425150ed926f9631a1316d5848c25f0a7c91eba69926eb530ecb``;
it has four parent ``cpu`` version-11 sections and four C910 version-1,
53-byte subsections.  The v2 stream is 9,551,435 bytes, SHA-256
``8a0ae58d5d97a6690900fb5cb7355a0670feec108c35a02dea224b44ab7aadb8``;
it has four parent version-12 sections and four C910 version-2, 61-byte
...
board suite passed 113/113 in the normal build and 112/112 in the
dependency-minimal build at that preceding storage checkpoint; the complete
13-test storage group also passed under ASan/UBSan.  The current FXCR/MMC-alias
checkpoint has since repeated the complete board and CSR gates in all three
build configurations, as recorded below.  Generic ARM
``xlnx-zcu102`` and PCI SDHCI libqos register paths also passed.  The standalone
NPCM SDHCI suite has not been rerun.

The generated DT now follows the pinned RevyOS/vendor-kernel aliases from
commit ``a092d55649279e1c9bcda2769b8f6b4370fa2c94``:
``mmc0 = &emmc``, ``mmc1 = &sdio0`` and ``mmc2 = &sdio1``.  A direct-DT qtest
checks all three paths.  The pinned upstream Ahead DTS at
``2709dd5ae32f0828f386327c76bba9f39f63a1c6`` omits MMC aliases, so this is a
vendor software-numbering compatibility choice, not a claim that mainline or
the owner's board has independently established the aliases.  Unchanged
pinned Linux boots in both normal and dependency-minimal QEMU and consequently
reports ``mmc0: new HS400 MMC card``.  A QEMU command trace proves its CMD6
...
A portable functional gate pins Linux 6.11.9 and a small RISC-V ext2 rootfs by
SHA-256.  It wraps the raw filesystem at sector 2048 in a deterministic MBR
whose disk signature is ``0x1520a110`` and boots partition 1 as
``root=PARTUUID=1520a110-01``.  The identifier is independent of asynchronous
MMC host probe order; the aliases above currently make the eMMC ``mmc0`` and
the partition ``mmcblk0p1``, but the gate does not rely on that device name.

With one hart, Linux enumerates HS400, mounts the eMMC image as root, enters a
controlled root shell, writes/syncs and hashes a deterministic 1 MiB file,
remounts read-only, then closes QEMU.  A fresh QEMU process reopens the same
scratch image and verifies the hash.  Normal and dependency-minimal builds
pass.  The corresponding four-hart qcow2-overlay gate also passes in both
builds: both its writer and fresh verifier observe CPUs 0-3 online, HS400 and
the ext2 root; the writer syncs the same payload and remounts read-only, and
the verifier reproduces SHA-256
``fb5ac1fab9c5b0e2b328bbe2149dee4664cf064618884f78c10e5c3bf6a05cda``
before another read-only remount.

These are deliberately storage-specific controlled-root-shell gates.  They do
not prove normal distro init, host-cache eviction, power-loss durability,
``e2fsck``, stress, an official board image or long-run freedom from an
intermittent scheduler/storage defect.

A separate four-hart audit retained the original one-off run in which CPU2
failed to come online and CPU0 later soft-locked during the kernel's unaligned-
access check.  Eight subsequent completed four-hart runs all brought up four
CPUs; a partial run labeled for single-threaded TCG also reached four CPUs
before ending prior to the MMC probe.  Most exploratory runs used
``root=179:0 rootwait``.  That is not a valid asynchronous-root gate: Linux
can resolve the numeric device number before the block device exists, causing
``rootwait`` to return while MMC discovery is still in flight.  The resulting
early root-open failures are a harness race.  That race does not itself explain
the mixed card-initialization results, so those runs neither isolate nor rule
out an intermittent QEMU storage defect.

Before the formal gate existed, one fresh qcow2-overlay control used the then
probe-order-dependent ``root=/dev/mmcblk1`` path.  It brought up all four CPUs,
enumerated the HS400 card, mounted ext2 and emitted
``FOUR_HART_EMMC_PASS``.  The later PARTUUID-based normal/minimal gates above
supersede that one path-only control and include the complete integrity and
fresh-process sequence.  The earlier CPU2-offline/CPU0-soft-lockup event was
not reproduced, but remains a local historical observation rather than a
physical-hardware fact or proof that indefinite stress cannot reproduce it.
...
The scalar/vector-permutation and slide-legality checkpoints close four
concrete RV64 wrong-result or illegal-encoding bugs and add mutation-sensitive
oracles, while the GMAC receive-interrupt watchdog closes the Linux driver's
immediate interrupt-mitigation dependency.  The follow-up vector-index
width/group regression independently checks ``th.vrgather.vv`` at e8,m2,
...
The AXI-DMAC width/address checkpoint independently validates the existing
memory-to-memory model's e16-source/e8-destination transfer and both fixed
...
| C910 vector | Missing upstream; this workspace has a separate XTheadVector decoder, 128-bit state, frozen v0.7.1-derived execution engine, CSRs, debug/migration support and architectural guests covering state, status, reduction, mask overlap, RV64 e8,m1 scalar permutation and slide legality, and e8,m2/e8,m8/e16,m1/e32,m1/e64,m1 vector gather indices | Complete per-instruction and randomized differential coverage across SEW/LMUL/XLEN, OS context/signal/ptrace tests, XTheadZvamo evidence and physical comparison without conflating it with RVV 1.0 |
...
| Ethernet | A reusable DWC GMAC 3.x model now provides descriptor DMA, IRQs, FCS, Clause 22 MDIO, a configurable PHY and VMState; both TH1520 instances and their APB glue are integrated, individual and shared AP reset groups drive resets, and mainline Linux binds GMAC0 as DWMAC1000. Receive filtering covers MAC0 plus 31 enable-controlled perfect addresses, byte masks and source selection; promiscuous/receive-all, broadcast/multicast, inverse and four control-frame modes; 64-bin unicast/multicast hashing; C-/S-VLAN exact, VID-only and inverse matching; and final-descriptor DA/SA/VLAN status. TH1520 additionally enables Type-2 RX status for a bounded IPv4/IPv6 TCP/UDP/ICMP subset. The shared transmit COE honors TXCOESEL, TSF and first-descriptor CIC0-3, inserts IPv4 header and IPv4/IPv6 TCP/UDP/ICMP checksums through one C-VLAN or ESVL-enabled S-VLAN, preserves guest source buffers, and writes terminal enhanced IHE/IPE/ES status. The low eight bits of DMA RIWT implement Linux's DIC-based RX interrupt mitigation at a fixed TH1520 500 MHz reset/reference rate; expiry raises RI/NIS, reset/zero/immediate RI cancel the timer, and VMState v2 preserves its deadline. Clean normal/minimal Linux one-/four-hart runs pass DHCP, 3/3 ping and a 1 MiB HTTP SHA-256 transfer | Validate the physical 32-entry/64-bin synthesis and exact filter, VLAN, control-frame and pause semantics; establish whether VLAN hash exists before implementing it; complete RX drop/forward threshold policy and malformed/zero-checksum corners; compare TX CIC2, CIC1-on-IPv6, malformed-length/status, trailing/padding, FIFO/PBL recovery, threshold, fragment/extension and stacked-VLAN behavior; add PTP/MMC/WOL/EEE, flow control, RTL8211F vendor pages/delays/IRQ/reset, traffic stress and error injection; validate the RIWT clock, DIC/reload/write/W1C/stop/reset behavior, dynamic clock/gate effects, individual/shared reset boundaries and whether they cover the embedded QEMU PHY |
| SPI/QSPI | A reusable DW APB SSI master is integrated at the Linux-described SPI0 node with its AP reset pair. The pinned mainline DT/driver tree supplies no SPI child or QSPI controller/programming contract. Vendor U-Boot's `light-beagle.dts` instead names an SPI0 JEDEC NOR and a QSPI1 NAND child, but the V1 board BOM contains neither part and design data only records boot-selection labels. QEMU deliberately attaches neither device, so QSPI/XIP is not inferred from clock/reset names alone | Reconcile the conflicting firmware/BOM/design evidence against the owner's PCB; validate the TH1520 synthesis, reset split and board wiring; add a default or opt-in flash only after its controller, part and routing contract is established |
...
* the C9xx PMU's 16 programmable counters, raw-selector WARL rules,
  ...
  loads synthetic, descriptor-exact C910 CSR VMState v1 and v2 subsections
  into poisoned destinations, including the v1-era parent CPU version 11.
...
* a reusable DesignWare APB SSI master at SPI0, ``0xffe700c000``, on PLIC
  ...
  board child.  Vendor U-Boot's ``light-beagle.dts`` describes SPI0 JEDEC NOR
  and QSPI1 NAND children, but the V1 board BOM contains neither flash part;
  those incompatible sources therefore do not justify a QEMU attachment.
  Standard, transmit-only, receive-only and EEPROM-read FIFO
...
* a reusable DesignWare GMAC 3.x model, factored from the NPCM implementation,
  ...
  only on the terminal descriptor with enhanced-format ES semantics.  The
  low eight bits of DMA RIWT provide the Linux-visible receive-interrupt
  watchdog: a DIC-suppressed completed frame arms a 256-cycle-unit one-shot,
  expiry raises RI/NIS, and an immediate non-DIC RI, a zero write or reset
  cancels it.  The reusable model defaults its conversion clock to zero;
  TH1520 supplies a fixed 500 MHz reset/reference-rate assumption, making
  Linux's default ``0xa0`` exactly 81,920 ns.  Current VMState version 2
  migrates an armed timer and pre-version-2 loads remain deliberately unarmed
  because no old deadline exists; and
* a reusable Synopsys DesignWare AXI DMAC 1.01a model with four channels,
  ...
  VMState.  Its focused qtest checks e16-source/e8-destination movement and
  fixed source/destination address modes with exact SAR/DAR results under the
  supported memory-to-memory transfer type.  The TH1520 general controller is
  mapped at ``0xffefc00000`` on
...
their seven available C910 CSR/migration subtests.  At the current checkpoint,
the dependency-minimal and ASan/UBSan board gates each pass
115/115.  The
...
normal, dependency-minimal and ASan/UBSan builds.  That initial dynamic
checkpoint is RV64 e8,m1 only; later vector-index checkpoints extend it while
other widths, LMULs, RV32, big-endian execution and silicon behavior remain
explicitly unverified under ``CPU-006``.

Checkpoint ``78ad4d6e56`` keeps the same sixth payload but adds two
independent boundaries.  The ``th.vrgather.vv`` helper now carries an e64
vector index as ``uint64_t`` instead of truncating it to 32 bits.  A scalar
e64,m1 oracle uses source values ``0x1122334455667788`` and
``0x99aabbccddeeff00`` with indices ``1ULL << 32`` and 1, requiring zero and
the second source value respectively.  The slide-down translator uses a
dedicated checker because frozen RVV 0.7.1 gives ``th.vslidedown.vx``,
``th.vslidedown.vi`` and ``th.vslide1down.vx`` an instruction-specific
masked-``v0`` destination prohibition which overrides the generic LMUL=1
overlap allowance.  Three raw encodings must trap, with the first preserving
nonzero ``vstart`` and every ``v0`` byte; unmasked ``v0`` destinations and
masked non-``v0`` destinations remain legal.  Restoring the 32-bit vector
index or the generic overlap checker fails at exits 14 and 15.  The augmented
six-payload set passes directly in normal, dependency-minimal and ASan/UBSan
builds.  Its dynamic permutation scope is RV64 e8,m1 scalar and e64,m1 vector
indexing, not all SEW/LMUL combinations; RV32, big-endian execution, remaining
forms and physical C910 comparison stay open under ``CPU-006``.

Checkpoint ``6d147eea4b`` extends that independent vector-index oracle rather
than changing the CPU model.  It checks e8,m2 (32 lanes over a two-register
group), e16,m1 and e32,m1 in addition to e64,m1.  Each case checks an in-range
unsigned vector index and an out-of-range result of zero.  The focused guest
passes in normal, dependency-minimal and ASan/UBSan builds.  This is QEMU
regression coverage, not a physical-C910 result; untested SEW/LMUL/XLEN
combinations remain open.

Checkpoint ``4f9c7e9893`` adds the legal e8,m8 case.  Its 128-lane scalar
oracle uses separate, group-aligned source ``v8-v15``, index ``v16-v23`` and
destination ``v24-v31`` groups, and checks in-range plus out-of-range indices
across every register boundary.  The focused guest passes in normal,
dependency-minimal and ASan/UBSan builds.  It is QEMU regression coverage, not
physical-C910 evidence.
...
enumerates the image as ``mmc0: new HS400 MMC card`` in both the normal and
dependency-minimal QEMU builds.  The generated DT explicitly follows vendor
commit ``a092d55649279e1c9bcda2769b8f6b4370fa2c94`` with
``mmc0=&emmc``, ``mmc1=&sdio0`` and ``mmc2=&sdio1``; the direct-boot DT qtest
checks every alias.  The pinned upstream DTS omits those aliases, so stock-DT
and hardware comparison remain open instead of treating the vendor convention
as an independently proved mainline contract.  A command trace records HS200,
HS, DDR8 and HS400 CMD6 updates but no CMD21.  The pinned TH1520 callback
explicitly skips CMD21 during HS400 preparation, so this gate proves EXT_CSD
...
qcow2-overlay test pass in normal and dependency-minimal builds.  Each writer
mounts eMMC root, writes and syncs a deterministic 1 MiB payload, verifies
SHA-256, remounts read-only and closes QEMU; a fresh QEMU process mounts the
same disk, reproduces the hash and remounts read-only.  The SMP writer and
verifier additionally require CPUs 0-3 online.  This is bounded functional
coverage, not ``e2fsck``, block stress, cache eviction, power-loss durability,
normal distro init or official-image coverage.  CQE, SDIO Wi-Fi, physical-card
GPIOs and mask-ROM boot behavior remain unproved.
...
is not a valid storage regression result.  A pre-alias fresh-overlay control
using ``root=/dev/mmcblk1`` brought up four CPUs, enumerated HS400, mounted ext2
and reached its controlled marker.  The later PARTUUID-based normal/minimal
four-hart gates supersede that control and complete the integrity/reopen
sequence.  A sufficiently long single-threaded-TCG control and broader stress
still remain open.
...
driver/register contract.  It also enables RX mitigation through DMA RIWT and
programs the Linux default ``0xa0``.  QEMU converts that value using a fixed
...
``run-9uieaud1`` and ``run-tk0021lq`` cover normal one- and four-hart Linux;
``run-ev_lpw_x`` and ``run-xc7n_lma`` repeat both with the dependency-minimal
QEMU.  A separate ``run-53c1sfu1`` is intentionally retained as a
contention/timing-sensitive failure: DHCP and three pings passed and the HTTP
GET reached the host, but RIWT expired while Linux NAPI had RIE masked; an
unrelated TX interrupt then made the ISR W1C-clear the masked RI without
scheduling RX, and three completed descriptors were temporarily stranded.
The 1 MiB transfer consequently stalled.  No QEMU correctness workaround was
added for this Linux masked-interrupt race.  The focused qtests establish
QEMU's deterministic CIC0-3 transmit and RIWT contracts, while the clean
...
Pinned openC910 RTL also defines user CSR ``FXCR`` at 0x800 as a 64-bit
floating-point control/status alias.  It resets to zero, is illegal while FS
is Off, and an otherwise legal write marks FS Dirty.  Its software-visible
mask is ``0x0780003f``: bits 26:24 alias ``frm``; bit 23 is DQNaN, where zero
selects the default/canonical NaN and one propagates a source NaN; bit 5 is a
writable flag that otherwise sticky-sets on a floating-point exception event;
and bits 4:0 alias ``fflags``.  All other bits read zero and ignore writes.  The
...
checks M/S/U access with FS Off and enabled, read-versus-write FS transitions,
the exact mask/aliases, scalar H/S/D and XTheadVector canonical-versus-source
NaNs, all five IEEE exception classes, direct flag writes, and a new occurrence
...
outer ``QEVM`` stream remains version 3.  The v1 transformation also changes
the four parent ``cpu`` section headers from version 12 to their historical
version 11; v2 retains version 12.  These controlled transformations isolate
the CPU compatibility path and complement, rather than impersonate, the
genuine producer captures below.
...
At this historical migration checkpoint, the complete normal and
dependency-minimal board/CSR gates passed 114/114 plus 14/14 and 113/113 plus
7/7 respectively.  Its preceding complete ASan/UBSan board gate passed
112/112, with the focused whole-machine, legacy-device and eMMC tuning
migration cases checked separately.
...
is armed and fails the expanded payload at stage 45, the repeated already-
sticky NX case; the correct binary passes.  The preserved mutant is
...
A separate targeted mutation which removes the raised-event arm from
``riscv_cpu_check_fflags()`` fails at stage 46, where quiet-NaN ``flt.s``
...
``../validation-artifacts/c910-historical-migration-20260825`` builds and pins
the last version-1 and version-2 producers, runs a small TCG guest which writes
distinct values on all four harts, captures their streams, and decodes every
C910 payload field.  Version 1 producer ``cce292534e6c`` emitted four parent
CPU-v11 sections and four 53-byte C910-v1 subsections; version 2 producer
``10321e0df167`` emitted four parent CPU-v12 sections and four 61-byte C910-v2
...
indices, a partial source-boundary crossing, in-place execution, full-width
  ...
  wrap, truncation and off-by-one exits.  The same payload now also checks an
...
comparison pass; at that migration checkpoint the complete board gates passed
114/114 in the normal build and 113/113 in the dependency-minimal build.  Its
preceding complete ASan/UBSan board gate passed 112/112, and the focused
...
power-loss behavior remain open.  The initial
Ethernet
...
open without adding a QEMU workaround.  CQE/ADMA3 and
...
its qtests include e16-to-e8 and fixed-address direct memory transfers, and
all-channel Linux ``dmatest`` passes.  That boundary remains a memory-to-memory
model result: peripheral handshakes/request routing, dynamic and other
...
RIWT still needs owner-board comparison of the actual clock, DIC start/reload
edges, W1C and live-write behavior, DMA stop/disable, clock-rate/gate changes,
reset domains and expiry-to-PLIC latency on both cores.  The fixed 500 MHz QEMU
conversion is a documented reset/reference-rate assumption, not a dynamic
silicon clock model.
